...
| Таблица          | СУБД / Хранилище                  | Поля (тип / размер)                                                                                                                                                                     | Индексы / уникальные ключи                                            | Шардирование / резервирование               | Денормализация / кэш                                            | Клиентские библиотеки / интеграции            | Балансировка / мультиплексирование    | Бэкап / восстановление                                       |
| ---------------- | --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- | ------------------------------------------- | --------------------------------------------------------------- | --------------------------------------------- | ------------------------------------- | ------------------------------------------------------------ |
| `user`           | PostgreSQL + S3                   | id (UUID), name (VARCHAR), email (VARCHAR), password_hash (CHAR32), avatar_url (VARCHAR), banner_url (VARCHAR), country (CHAR2), gender (CHAR1), created_at (DATETIME), birthday (DATE) | PK(id), UNIQUE(email)                                                 | Шардирование по id, master-slave репликация | Кэш профилей в Redis, S3 для аватаров и баннеров                | psycopg2 / asyncpg, boto3                     | PgBouncer / HAProxy                   | Full backup daily + incremental 6h, S3 георепликация         |
| `video`          | PostgreSQL + ElasticSearch + S3   | id (UUID), title (VARCHAR), author_id (UUID), description (TEXT), created_at, updated_at, thumbnail_url (VARCHAR), duration (FLOAT)                                                     | PK(id), INDEX(title), INDEX(author_id)                                | Шардирование по author_id, read-replicas    | Кэш метаданных в Redis, видео хранится в S3, ElasticSearch для поиска/рекомендаций | psycopg2, Elasticsearch-py, boto3             | PgBouncer + Elastic client pooling    | Full backup + incremental; S3 + Elastic snapshots            |
| `chunk`          | PostgreSQL + S3/CDN               | id (UUID), video_id (UUID), quality (VARCHAR), chunk_index (INT), url (VARCHAR), size (INT), duration (FLOAT)                                                                           | PK(id), INDEX(video_id, chunk_index)                                  | Шардирование по video_id, master-slave      | CDN кэширует чанки, Redis для метаданных                        | psycopg2 / boto3                              | PgBouncer + CDN edge caching          | Георепликация в S3, регулярное integrity check               |
| `comment`        | PostgreSQL                        | id (UUID), text (TEXT), video_id (UUID), author_id (UUID), created_at, updated_at, parental_comment_id (UUID)                                                                           | PK(id), INDEX(video_id), INDEX(author_id), INDEX(parental_comment_id) | Шардирование по video_id, master-slave      | Топ-комментарии кэшируются в Redis                              | psycopg2 / asyncpg                            | PgBouncer                             | Full + incremental backup, репликация                        |
| `video_reaction` | ClickHouse + PostgreSQL           | id (UUID), author_id (UUID), video_id (UUID), is_upvote (BOOLEAN), created_at (DATETIME)                                                                                                | PK(id), INDEX(video_id, author_id)                                    | Шардирование по video_id, репликация        | Счетчики лайков в Redis                                         | clickhouse-driver, psycopg2                   | ClickHouse load balancing + PgBouncer | ClickHouse snapshots + PostgreSQL backup                     |
| `view`           | ClickHouse + PostgreSQL + ElasticSearch | id (UUID), user_id (UUID), video_id (UUID), watched_at (DATETIME), watch_duration (FLOAT)                                                                                         | PK(id), INDEX(video_id, user_id)                                      | Шардирование по video_id, репликация        | Агрегаты просмотров в Redis, Elastic для поиска/аналитики       | clickhouse-driver, psycopg2, Elasticsearch-py | Load-balanced ClickHouse + PgBouncer  | ClickHouse snapshots + PostgreSQL backup + Elastic snapshots |
| `subscription`   | PostgreSQL                        | id (UUID), channel_id (UUID), subscriber_id (UUID), created_at (DATETIME)                                                                                                               | PK(id), INDEX(channel_id, subscriber_id)                              | Шардирование по channel_id, master-slave    | Кэш популярных подписок в Redis                                 | psycopg2 / asyncpg                            | PgBouncer                             | Full + incremental backup, репликация                        |
