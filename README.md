...
| ------------------------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Kubernetes / микросервисы**         | Репликация pod’ов (ReplicaSets), Auto-healing, Rolling updates       | Kubernetes автоматически перезапускает контейнеры при сбое, перераспределяет нагрузку между узлами и обновляет сервисы без простоя. Это гарантирует стабильную работу API, видео- и ML-сервисов при обновлениях или сбоях.     |
...
| **L3/L4 балансировщики (DR схема)**   | N+1 резервирование, keepalived/VRRP                                  | Балансировщики работают по схеме Direct Routing. При сбое активного — VRRP назначает резервный. Производительность сохраняется, обратный трафик идёт напрямую.                                                                 |
...
| **PostgreSQL**                        | Master–Slave репликация, Patroni failover, full + incremental backup | Основные таблицы (user, video, comment и др.) реплицируются на несколько узлов. Patroni выполняет автоматическое переключение на slave при отказе master. Резервные копии создаются ежедневно и инкрементально каждые 6 часов. |
