...
1. **Маршрутизаторы BGP/RIP**

   * BGP распределяет трафик между балансировщиками L7 по оптимальным маршрутам.
   * RIP/OSPF – менее гибкие, ограниченные в масштабировании.
2. **Балансировщик L7 (Envoy/Nginx)**

   * Выполняет SSL Termination.
   * Кеширует некоторые запросы.
   * Использует алгоритмы распределения нагрузки:

     * Weighted Round Robin для низкой интенсивности.
     * Least Connections для пиковых нагрузок (например, потоковое видео).
   * Ре-try стратегии с jitter для снижения коллизий.
3. **Failover / Healthcheck**

   * Healthcheck на уровне Kubernetes или встроенный в балансировщик.
   * Репликация серверов и кластеров для отказоустойчивости.
   * Таймауты на основе квантилей (например, 95% percentile).
...
1. **SSL Termination**
   ...
   * Планируем загрузку максимум на 70% от реальной мощности.
