...
## 10. Схема проекта
<img width="864" height="624" alt="structure (1)" src="https://github.com/user-attachments/assets/76e520d5-0a4b-4943-88c1-92fcc9424c2b" />
...
| Хранилище       | Пиковый RPS (чтение) | Пиковый RPS (запись) |
|------------------|----------------------|-----------------------|
| PostgreSQL       | 416 668              | 14 493                |
| Elasticsearch    | 101 273              | 108                   |
| ClickHouse       | 208 334              | 108 334               |
| S3               | 104 275              | 216                   |

### PostgreSQL

Согласно расчетам, нагрузка на кластер PostgreSQL составляет:

* **Запись:** 14 493 RPS (пиковая)
* **Чтение:** 416 668 RPS (пиковая)
* **Оценка TPS:** ≈ **14 500 TPS**

Практически:

* один мощный PostgreSQL-инстанс (16–32 vCPU, NVMe) устойчиво выдерживает:
  * **3–6 тыс. TPS записи**
  * **50–150 тыс. SELECT RPS**

Текущая нагрузка:
* **14.5k TPS записи → не помещается в один инстанс**
* **416k RPS чтения → требует агрессивного масштабирования по репликам**

### Принятая конфигурация PostgreSQL:

* **Минимум:**
  * **3 master-шарда**
  * **на каждый — 2–3 read-реплики**
* **Итого:** **9–12 узлов PostgreSQL**
* На каждый master:
  * **32 CPU**
  * **128 ГБ RAM**
  * **NVMe SSD**
...
* S3 **не требует ручного масштабирования** по RPS,
* используется:
...
Ограничения здесь связаны **не с RPS, а со стоимостью egress-трафика**.

### Итого

| Сервис / Хранилище  | CPU (cores) | RAM (ГБ) | NVMe (ТБ) | Кол-во серверов |
|----------------------|-------------|----------|-----------|------------------|
| **Stateless-сервисы (K8s)** | 32          | 64       | 1         | **164**          |
...
| **Elasticsearch**        | Bare Metal                |
| **ClickHouse**           | Bare Metal                |
| **Amazon S3**            | Managed Cloud Service     |
