...
| Таблица            | СУБД / хранение                   | Типы данных / поля                                                                                                                                                           | Индексы                                                               | Шардирование         | Денормализация                                                                | Клиентские библиотеки / интеграции                                    | Балансировка / мультиплексирование                             | Резервное копирование / репликация                                         | Реплики / количество нод                                                                |
| ------------------ | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- | -------------------- | ----------------------------------------------------------------------------- | --------------------------------------------------------------------- | -------------------------------------------------------------- | -------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| **user**           | PostgreSQL + S3                   | `id UUID, name VARCHAR, email VARCHAR, password_hash CHAR(32), avatar_url VARCHAR, banner_url VARCHAR, country CHAR(2), gender CHAR(1), created_at TIMESTAMP, birthday DATE` | PK(id), UNIQUE(email)                                                 | hash по `id`         | хранение количества подписчиков                                               | `pgx`, `go-sqlx`, `minio-go`                                          | PgBouncer для соединений                                       | full backup daily + incremental каждые 6 ч, репликация master–slave        | Master: 1, Read-replicas: 3–4                                                           |
| **video**          | PostgreSQL + Elasticsearch + S3   | `id UUID, title VARCHAR, author_id UUID, description TEXT, created_at TIMESTAMP, updated_at TIMESTAMP, thumbnail_url VARCHAR, duration FLOAT`                                | PK(id), INDEX(title), INDEX(author_id)                                | hash по `author_id`  | хранение просмотров, лайков, аватара, имени, фото автора для быстрого доступа | `pgx`, `olivere/elastic` (или `elastic/go-elasticsearch`), `minio-go` | master + несколько read-replicas (через HAProxy)               | full backup + incremental, репликация PostgreSQL + S3 versioning           | Postgres Master: 1, Read-replicas: 3; ES nodes: 3                                       |
| **chunk**          | PostgreSQL + S3                   | `id UUID, video_id UUID, quality VARCHAR, chunk_index INT, url VARCHAR, size INT, duration FLOAT`                                                                            | PK(id), INDEX(video_id, chunk_index)                                  | hash по `video_id`   | хранение чанков на S3 для CDN                                                 | `pgx`, `minio-go`                                                     | read-replicas через connection pooler                          | S3 cross-region replication, регулярный бэкап метаданных                   | Postgres Master: 1, Read-replicas: 2–3                                                  |
...
| **subscription**   | PostgreSQL                        | `id UUID, channel_id UUID, subscriber_id UUID, created_at TIMESTAMP`                                                                                                         | PK(id), INDEX(channel_id, subscriber_id)                              | hash по `channel_id` | —                                                                             | `pgx`, `go-sqlx`                                                      | PgBouncer connection pooling                                   | full backup + logical replication                                          | Master: 1, Read-replicas: 2–3                                                           |
| **view**           | ClickHouse + PostgreSQL + Elastic | `id UUID, user_id UUID, video_id UUID, watched_at TIMESTAMP, watch_duration FLOAT`                                                                                           | PK(id), INDEX(video_id, user_id)                                      | hash по `video_id`   | агрегаты просмотров для быстрого доступа                                      | `ClickHouse Go driver`, `pgx`, `elastic/go-elasticsearch`             | ClickHouse cluster + PgBouncer for Postgres + ES load balancer | ClickHouse replication (Zookeeper) + PostgreSQL backup + Elastic snapshots | ClickHouse: Shards 4, Replicas 2 (8 нод), Postgres Master 1 + 2–3 replicas, ES nodes: 3 |
