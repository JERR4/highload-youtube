...
| **Алгоритм персонализированных рекомендаций**      | Получение списка рекомендаций       | **Назначение:** увеличивать вовлечённость пользователей и удержание на платформе. <br> **Принцип работы:** <br> 1. Из базы данных (или аналитического слоя) извлекаются события: <br> Просмотрено видео (views) <br> Лайкнуто видео (likes) <br> Комментарий к видео (comments) <br> Подписка на автора (subscriptions)<br> Каждое событие может имеет вес (например, просмотр = 1, лайк = 3, подписка = 5). <br> 2. Создаётся матрица, где строки — пользователи, столбцы — видео, значения — уровень интереса (веса взаимодействий). <br> 3. Для нахождения пользователей со схожими интересами вычисляется метрика сходства. <br> 4. Для конкретного пользователя (например, U1) ищутся наиболее похожие пользователи (U2, U3). Анализируются видео, которые они смотрели или лайкали, но U1 — нет. Рассчитывается взвешенная оценка релевантности видео с учётом сходства. <br> 5. Выдача рекомендаций на фронтенд. |                                                                         |
