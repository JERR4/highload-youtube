...
| Таблица        | СУБД / хранение                         | Типы данных / поля                                                                                                                                                         | Индексы                                                               | Шардирование       | Денормализация / кеш                                                     | Клиентские библиотеки / интеграции                            | Балансировка / мультиплексирование                             | Резервное копирование / репликация                                                                                         | Реплики / количество нод                                                                |
| -------------- | --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- | ------------------ | ------------------------------------------------------------------------ | ------------------------------------------------------------- | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------- |
| user           | PostgreSQL + S3                         | id UUID, name VARCHAR, email VARCHAR, password_hash CHAR(32), avatar_url VARCHAR, banner_url VARCHAR, country CHAR(2), gender CHAR(1), created_at TIMESTAMP, birthday DATE | PK(id), UNIQUE(email)                                                 | hash по id         | хранение количества подписчиков для быстрого доступа                     | pgx, go-sqlx, minio-go                                        | PgBouncer для соединений                                       | PostgreSQL: full backup daily + incremental каждые 6 ч, репликация master–slave; S3: versioning + cross-region replication | Master: 1, Read-replicas: 3–4                                                           |
| video          | PostgreSQL + Elasticsearch + S3         | id UUID, title VARCHAR, author_id UUID, description TEXT, created_at TIMESTAMP, updated_at TIMESTAMP, thumbnail_url VARCHAR, duration FLOAT                                | PK(id), INDEX(title), INDEX(author_id)                                | hash по author_id  | хранение просмотров, лайков, аватара и имени автора для быстрого рендера | pgx, olivere/elastic (или elastic/go-elasticsearch), minio-go | Master + несколько read-replicas через HAProxy                 | PostgreSQL: full + incremental backup, репликация; S3: versioning; Elasticsearch: snapshot + replication                   | Postgres Master: 1, Read-replicas: 3; ES nodes: 3                                       |
| chunk          | PostgreSQL + S3                         | id UUID, video_id UUID, quality VARCHAR, chunk_index INT, url VARCHAR, size INT, duration FLOAT                                                                            | PK(id), INDEX(video_id, chunk_index)                                  | hash по video_id   | хранение метаданных чанков; сами файлы на S3 для CDN                     | pgx, minio-go                                                 | read-replicas через connection pooler                          | PostgreSQL: full + incremental backup, репликация; S3: cross-region replication                                            | Postgres Master: 1, Read-replicas: 2–3                                                  |
| comment        | PostgreSQL                              | id UUID, text TEXT, video_id UUID, author_id UUID, created_at TIMESTAMP, updated_at TIMESTAMP, parental_comment_id UUID                                                    | PK(id), INDEX(video_id), INDEX(author_id), INDEX(parental_comment_id) | hash по video_id   | топ-комментарии кэшируются                                               | pgx, go-sqlx                                                  | Master + read-replicas с автоматическим failover               | full + incremental backup, master–slave репликация                                                                         | Master: 1, Read-replicas: 3–4                                                           |
| subscription   | PostgreSQL                              | id UUID, channel_id UUID, subscriber_id UUID, created_at TIMESTAMP                                                                                                         | PK(id), INDEX(channel_id, subscriber_id)                              | hash по channel_id | хранение количества подписчиков на канале для быстрого доступа           | pgx, go-sqlx                                                  | PgBouncer connection pooling                                   | full backup + logical replication                                                                                          | Master: 1, Read-replicas: 2–3                                                           |
| view           | ClickHouse + PostgreSQL + Elasticsearch | id UUID, user_id UUID, video_id UUID, watched_at TIMESTAMP, watch_duration FLOAT                                                                                           | PK(id), INDEX(video_id, user_id)                                      | hash по video_id   | агрегаты просмотров для быстрого доступа                                 | ClickHouse Go driver, pgx, elastic/go-elasticsearch           | ClickHouse cluster + PgBouncer for Postgres + ES load balancer | ClickHouse: replication через Zookeeper; PostgreSQL: backup; Elasticsearch: snapshots                                      | ClickHouse: Shards 4, Replicas 2 (8 нод), Postgres Master 1 + 2–3 replicas, ES nodes: 3 |
| video_reaction | ClickHouse + PostgreSQL                 | id UUID, author_id UUID, video_id UUID, is_upvote BOOLEAN, created_at TIMESTAMP                                                                                            | PK(id), INDEX(video_id, author_id)                                    | hash по video_id   | счетчики лайков/дизлайков кэшируются                                     | ClickHouse Go driver, pgx                                     | ClickHouse cluster balancing (shards + replicas)               | ClickHouse: replicated tables; PostgreSQL: logical replication                                                             | ClickHouse: Shards 4, Replicas 2 (8 нод), Postgres Master 1 + 2 replicas, ES nodes: 3   |
