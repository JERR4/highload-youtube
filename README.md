...
## **11.1 Список серверов с целевой нагрузкой**

Ниже представлен расчёт по сервисам: пиковая нагрузка, тип обработки, требования CPU/RAM и обоснование выбора количества нод.

---

### **Stateless-сервисы (Kubernetes)**

| Сервис                          | Описание                        | Пиковая нагрузка (RPS) | Тип обработки   | Пиковое потребление CPU (ядра) | Пиковое потребление RAM (ГБ) |
| ------------------------------- | ------------------------------- | ---------------------- | --------------- | ------------------------------ | ---------------------------- |
| API (просмотр, лайки, комменты) | Обработка событий пользователей | 213 000                | Лёгкие операции | ~96                            | ~48                          |
| Search API (gateway)            | Прокси-запросы в Elasticsearch  | 101 000                | Лёгкая логика   | ~48                            | ~24                          |
| Recommendation service          | Персональные рекомендации       | 104 000                | Средняя логика  | ~2100                          | ~256                         |
| Upload Service                  | Загрузка медиа                  | 500                    | Средняя логика  | ~4                             | ~8                           |
| Kafka stateless helpers         | Продусеры / коннекторы          | 2 000                  | Лёгкая логика   | ~16                            | ~16                          |

---

### **Stateful-кластера**

| Кластер           | Описание                   | Пиковая нагрузка | Тип обработки            | Пиковое потребление CPU (ядра) | Пиковое потребление RAM (ГБ) |
| ----------------- | -------------------------- | ---------------- | ------------------------ | ------------------------------ | ---------------------------- |
| **Elasticsearch** | Поиск по гигабайтам текста | ~60k RPS         | Лёгкая логика, I/O-bound | ~200                           | ~960                         |
| **PostgreSQL**    | Транзакционные операции    | ~30k RPS         | Средняя логика           | ~192                           | ~768                         |
| **ClickHouse**    | Хранилище аналитики        | ~40k events/sec  | Средняя логика           | ~128                           | ~512                         |
| **Redis**         | Кеширование                | ~200k ops/sec    | Лёгкие операции          | ~32                            | ~256                         |
| **Kafka**         | Событийный стриминг        | ~20M events/day  | Лёгкая логика, диск      | ~32                            | ~128                         |

---

## **11.2 Выбор серверов и нод, конфигурации, суммирование ресурсов**

### **Kubernetes-нагрузка (stateless)**

Расчёт узлов K8s основан на выборе конкретной конфигурации:

**Тип воркер-ноды:** 48 vCPU / 300 ГБ RAM
(приближённый аналог Hetzner PX)

---

### **ИТОГО ресурсы сервисов внутри Kubernetes**

| Сервис                 | CPU           | RAM             | Требуемое кол-во нод                     |
| ---------------------- | ------------- | --------------- | ---------------------------------------- |
| API                    | ~96           | ~48             | 2                                        |
| Search API             | ~48           | ~24             | 1                                        |
| Recommendation service | ~2100         | ~256            | 44                                       |
| Upload                 | ~4            | ~8              | 1                                        |
| Kafka helpers          | ~16           | ~16             | 1                                        |
| **ИТОГО**              | **~2264 CPU** | **~352 ГБ RAM** | **~49 нод (расчётно) → округляем до 50** |

**Окончательное число Kubernetes worker nodes: 50**

---

### **Stateful-ноды**

| Тип узлов                              | Кол-во | CPU суммарно | RAM суммарно |
| -------------------------------------- | ------ | ------------ | ------------ |
| **Elasticsearch**                      | 9      | ~200         | ~960 ГБ      |
| **PostgreSQL**                         | 12     | ~192         | ~768 ГБ      |
| **ClickHouse**                         | 8      | ~128         | ~512 ГБ      |
| **Redis**                              | 8      | ~32          | ~256 ГБ      |
| **Kafka**                              | 4      | ~32          | ~128 ГБ      |
| **Service Nodes (оркестрация, CI/CD)** | 3      | ~24          | ~96 ГБ       |
...
| Тип узлов                   | Кол-во | CPU суммарно | RAM суммарно   |
| --------------------------- | ------ | ------------ | -------------- |
| **Kubernetes worker nodes** | **50** | **~2400**    | **~15 000 ГБ** |
| Elasticsearch               | 9      | ~200         | ~960           |
| PostgreSQL                  | 12     | ~192         | ~768           |
| ClickHouse                  | 8      | ~128         | ~512           |
| Redis                       | 8      | ~32          | ~256           |
| Kafka                       | 4      | ~32          | ~128           |
| Service nodes               | 3      | ~24          | ~96            |

---

## **11.3 Модель размещения серверов и стоимость**

Для выполнения задания указаны провайдеры и модель размещения.

### **Выбор провайдеров**

* **Kubernetes Cluster (worker nodes):** Hetzner (аренда)
* **Stateful (ES, PG, CH, Redis, Kafka):** собственные серверы (колокация)
* **Service Nodes:** Hetzner (аренда)

---

### **Месячная стоимость**

| Категория                           | Размещение      | Кол-во | Цена / мес | Суммарно / мес      |
| ----------------------------------- | --------------- | ------ | ---------- | ------------------- |
| **Kubernetes worker nodes (50 шт)** | Аренда, Hetzner | 50     | 250$       | **12 500$**         |
| Elasticsearch                       | Собственные     | 9      | 150$       | 1 350$              |
| PostgreSQL                          | Собственные     | 12     | 140$       | 1 680$              |
| ClickHouse                          | Собственные     | 8      | 125$       | 1 000$              |
| Redis                               | Собственные     | 8      | 50$        | 400$                |
| Kafka                               | Собственные     | 4      | 85$        | 340$                |
| Service nodes                       | Аренда, Hetzner | 3      | 60$        | 180$                |
| **ИТОГО**                           |                 |        |            | **17 450$ / месяц** |
